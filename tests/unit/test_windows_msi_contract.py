...
def test_per_machine_shortcuts_use_common_folders_and_machine_keypaths() -> None:
    ...
    root = ElementTree.parse(source).getroot()
    namespace = {"w": "http://schemas.microsoft.com/wix/2006/wi"}
    assert root.find(".//w:Directory[@Id='CommonProgramMenuFolder']", namespace) is not None
    assert root.find(".//w:Directory[@Id='CommonDesktopFolder']", namespace) is not None
